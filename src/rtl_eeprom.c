...
void rtl8101_eeprom_cleanup(void __iomem *ioaddr)
{
        ...
        rtl8101_raise_clock(&x, ioaddr);
        rtl8101_lower_clock(&x, ioaddr);
}

int rtl8101_eeprom_cmd_done(void __iomem *ioaddr)
{
        u8 x;
        int i;

        rtl8101_stand_by(ioaddr);
...
u16 rtl8101_eeprom_read_sc(struct rtl8101_private *tp, u16 reg)
{
        ...
        rtl8101_shift_out_bits(RTL_EEPROM_READ_OPCODE, 3, ioaddr);
        rtl8101_shift_out_bits(reg, addr_sz, ioaddr);

        data = rtl8101_shift_in_bits(ioaddr);

        rtl8101_eeprom_cleanup(ioaddr);
...
void rtl8101_eeprom_write_sc(struct rtl8101_private *tp, u16 reg, u16 data)
{
        ...
        rtl8101_shift_out_bits(RTL_EEPROM_EWEN_OPCODE, 5, ioaddr);
        rtl8101_shift_out_bits(reg, w_dummy_addr, ioaddr);
        rtl8101_stand_by(ioaddr);

        rtl8101_shift_out_bits(RTL_EEPROM_ERASE_OPCODE, 3, ioaddr);
        rtl8101_shift_out_bits(reg, addr_sz, ioaddr);
        if (rtl8101_eeprom_cmd_done(ioaddr) < 0) {
                return;
        }
        rtl8101_stand_by(ioaddr);

        rtl8101_shift_out_bits(RTL_EEPROM_WRITE_OPCODE, 3, ioaddr);
        rtl8101_shift_out_bits(reg, addr_sz, ioaddr);
        rtl8101_shift_out_bits(data, 16, ioaddr);
        if (rtl8101_eeprom_cmd_done(ioaddr) < 0) {
                return;
        }
        rtl8101_stand_by(ioaddr);

        rtl8101_shift_out_bits(RTL_EEPROM_EWDS_OPCODE, 5, ioaddr);
        rtl8101_shift_out_bits(reg, w_dummy_addr, ioaddr);
